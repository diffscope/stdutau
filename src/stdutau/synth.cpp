...
namespace Utau {
    ...
    void Synth::calc(int noteCount, const std::pair<int, int> &range, const NoteGetter &noteGetter,
                     const GenonSettingsGetter &genonSettingsGetter,
                     std::vector<std::pair<ResamplerArguments, WavtoolArguments>> *result) {

        int left = std::max(0, range.first);
        int right = std::min(noteCount - 1, range.second);
